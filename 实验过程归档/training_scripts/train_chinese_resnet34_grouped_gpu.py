# -*- coding: utf-8 -*-
"""Train ResNet34 Chinese gesture model on grouped train/test txt.

ResNet34 is a classic, robust ImageNet-pretrained baseline for comparison
against EfficientNet-B0. The original guide CNN models are left untouched.
"""
...
MODEL_PATH = ROOT / "models" / "chinese_resnet34_grouped_0_9.pth"
METRICS_PATH = ROOT / "outputs" / "chinese_resnet34_grouped_metrics.json"
IMAGE_SIZE = 224
...
def main() -> None:
    ...
    train_loader = DataLoader(train_dataset, batch_size=72, shuffle=True, num_workers=2, pin_memory=True, persistent_workers=True)
    ...
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=9.0e-4, weight_decay=2e-4)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if test_acc > best_acc:
            ...
            stale_epochs = 0
            best_report = {
                "model": "resnet34_chinese_grouped_0_9",
            ...
            print(f"Saved best ResNet34: {MODEL_PATH} ({best_acc:.2%})", flush=True)
        else:
            stale_epochs += 1
