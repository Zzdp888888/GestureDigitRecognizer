# -*- coding: utf-8 -*-
"""Train EfficientNet-B0 Chinese gesture model on grouped train/test txt.

EfficientNet-B0 is a stronger high-resolution classifier than MobileNetV3
while still fitting comfortably on an RTX 3070 Ti Laptop GPU. The original
guide CNN models are left untouched.
"""
...
MODEL_PATH = ROOT / "models" / "chinese_efficientnet_b0_grouped_0_9.pth"
METRICS_PATH = ROOT / "outputs" / "chinese_efficientnet_b0_grouped_metrics.json"
IMAGE_SIZE = 224
...
def build_model(weights=EfficientNet_B0_Weights.IMAGENET1K_V1):
...
def train_transform():
    return transforms.Compose(
        [
            ...
            transforms.RandomResizedCrop(IMAGE_SIZE, scale=(0.58, 1.0), ratio=(0.74, 1.28)),
            transforms.RandomAffine(degrees=20, translate=(0.14, 0.14), scale=(0.78, 1.18), shear=(-7, 7)),
            transforms.ColorJitter(brightness=0.38, contrast=0.38, saturation=0.25, hue=0.025),
            ...
            transforms.ToTensor(),
            transforms.RandomErasing(p=0.22, scale=(0.015, 0.09), ratio=(0.45, 2.3), value="random"),
...
def main() -> None:
    ...
    train_loader = DataLoader(train_dataset, batch_size=80, shuffle=True, num_workers=2, pin_memory=True, persistent_workers=True)
    ...
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1.0e-3, weight_decay=2e-4)
    ...
    for epoch in range(1, epochs + 1):
        if epoch == 4:
            ...
            optimizer = torch.optim.AdamW(model.parameters(), lr=2.0e-4, weight_decay=2e-4)
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs - 3, eta_min=6e-6)
        ...
        if test_acc > best_acc:
            ...
            stale_epochs = 0
            best_report = {
                "model": "efficientnet_b0_chinese_grouped_0_9",
            ...
            torch.save({"model_state": {k: v.detach().cpu() for k, v in model.state_dict().items()}, **best_report}, MODEL_PATH)
            print(f"Saved best EfficientNet-B0: {MODEL_PATH} ({best_acc:.2%})", flush=True)
        else:
            stale_epochs += 1
